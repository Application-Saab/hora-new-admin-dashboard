...
const AddMoreImages = () => {
  ...
  const handleUpload = async () => {
    ...
    const uploadPromises = selectedFiles.map(async (file) => {
      ...
      try {
        await axios.post('https://horaservices.com:3000/api/photo/upload', formData);
  ...
  return (
    <div style={{ padding: '20px', maxWidth: '600px' }}>
      ...
      {folderName && <p><strong>Folder Name:</strong> {folderName}</p>}
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', marginTop: '20px' }}>
            {selectedFiles.map((file, index) => (
              ...
              >
                ...
                />
                <p style={{ fontSize: '12px' }}>{uploadStatuses[file.name] || 'Pending'}</p>
              </div>
            ))}
          </div>
        </div>
