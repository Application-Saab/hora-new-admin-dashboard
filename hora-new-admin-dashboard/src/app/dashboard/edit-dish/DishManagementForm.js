...
// const DishManagementForm = ({ initialData }) => {
  ...
  useEffect(() => {
    if (initialData) {
      setFormData({
        dishName: initialData.name,
        dishImage: initialData.image,
        vegNon: initialData.is_dish ? '1' : '0',
        dishType: initialData.serving_dish.length > 0 ? 'serving-dish' : 'main-dish',
        category: initialData.catId,
        cuisineType: initialData.cuisineId,
        mealType: initialData.mealId,
        dishRate: initialData.dish_rate,
        preparation: initialData.is_preparation ? 'true' : 'false',
        perPlateQuantity: initialData.per_plate_qty.qty,
...
const cuisineArray = [
    ...
    if (validateForm()) {
      const requestData = {
        ...
        // serving_dish: formData.servingDish.filter(id => id !== null),
        // ...(formData.dishType === 'serving-dish' && { serving_dish: formData.servingDish.filter(id => id !== null) }), // Include serving_dish only if dishType is serving-dish
        serving_dish: formData.dishType === 'serving-dish' ? formData.servingDish.filter(id => id !== null) : [], // Send empty array if dishType is not serving-dish
        ...
        special_appliance_id: formData.specialAppliance.filter(id => id !== null), // Filter out null values
        general_appliance_id: formData.generalAppliance.filter(id => id !== null), 
  ...
  console.log('editDish removed from localStorage');
      } catch (error) {
  ...
  return (
    <div className={styles["dish-form-container"]}>
      <h2 className={styles["form-header"]}>Edit Dish</h2>
