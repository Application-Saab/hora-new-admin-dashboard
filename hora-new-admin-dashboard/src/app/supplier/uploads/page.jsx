...
const CapsuleUpload = () => {
    ...
    const [deletingFile, setDeletingFile] = useState(null);
    const [galleryDetails, setGalleryDetails] = useState(null);
    const [isSubmittingDone, setIsSubmittingDone] = useState(false); // Done button loading state

    const searchParams = useSearchParams();
    const router = useRouter();

    const toId = typeof window !== "undefined" ? localStorage.getItem("supplierID") : null;
    const orderId = searchParams.get("orderId");
    ...
    const uploadEventCapsuleFile = async (file, index) => {
        try {
            ...
            // 1. Original extension nikalna (e.g., .jpg, .png)
            const fileExtension = file.name.substring(file.name.lastIndexOf('.'));

            // 2. Crypto se unique random string generate karna
            const uniqueId = crypto.randomUUID ? crypto.randomUUID() : Array.from(crypto.getRandomValues(new Uint8Array(16)))
                .map(b => b.toString(16).padStart(2, '0'))
                .join('');

            // 3. Unique file name (e.g., 9b1deb4d-3b7d-4bad-9bd2-2ca771800000.jpg)
            const uniqueFileName = `${uniqueId}${fileExtension}`;

            const response = await axios.post(
                `http://localhost:4000/get-event-capsule-presigned-url`,
                {
                    fileName: uniqueFileName, // 👈 Ab S3 presigned URL request me UNIQUE file name jaayega
                    fileType: file.type,
                    folderName: galleryDetails?.folderName,
                },
                {
            ...
            const originalUrl = `https://photography-hora.s3.eu-north-1.amazonaws.com/${key}`;
            ...
            return { success: true, key, originalUrl };
        } catch (error) {
            console.error(`Upload failed for ${file.name}:`, error);
    ...
    const handleEventCapsuleUpload = async (e) => {
        ...
        setEventCapsuleFiles((prev) => [...selectedFiles, ...prev]);
        setUploadingFiles(true);

        try {
            for (let i = 0; i < selectedFiles.length; i++) {
                await uploadEventCapsuleFile(selectedFiles[i].file, i);
            }
        } catch (error) {
            console.error("Event Capsule upload error:", error);
        } finally {
    ...
    const handleDeleteS3File = async (item, index) => {
        ...
        try {
            ...
            await axios.delete(`http://localhost:4000/delete-s3-image`, {
                headers: {
                    Authorization: `${token}`,
                },
                data: {
    ...
    const handleUploadDone = async () => {
        ...
        if (!folderId || !folderName || !orderId) {
            alert("Folder Details or Order ID missing.");
            return;
        }
        ...
        try {
            ...
            if (response.data?.success) {
                alert("Upload marked as complete! Background processing started.");
            } else {
                ...
                <>
                    ...
                    >
                        <input
                            ...
                            accept="image/*,video/*"
                            onChange={handleEventCapsuleUpload}
                            disabled={uploadingFiles || isSubmittingDone}
                            style={{ display: "none" }}
                        />
                        ...
                        <button
                            ...
                            style={{
                                ...
                                fontSize: "14px",
                                fontWeight: "600",
                                cursor: (uploadingFiles || isSubmittingDone) ? "not-allowed" : "pointer",
                                opacity: (uploadingFiles || isSubmittingDone) ? 0.7 : 1,
                            }}
                        >
                            {uploadingFiles ? "Uploading..." : "Upload Images / Videos"}
                        </button>

                        {/* Complete Upload (Done) Button */}
                        <button
                            type="button"
                        ...
                        >
                            {isSubmittingDone ? "Processing..." : "Complete & Submit Upload"}
                        </button>
                    </div>
                                ...
                                >
                                    ...
                                    {item.status === "uploaded" && (
                                        <button
                                            type="button"
                                            onClick={() => handleDeleteS3File(item, index)}
                                            disabled={deletingFile === index || isSubmittingDone}
                                            style={{
                                                position: "absolute",
                                                top: "5px",
                                                right: "5px",
                                    ...
                                    >
                                        {item.status === "pending" &&
                                            `${item.fileType?.startsWith("video/") ? "Video" : "Image"} - Pending`}

                                        {item.status === "uploading" &&
                                            `${item.fileType?.startsWith("video/") ? "Video" : "Image"} - Uploading ${item.progress || 0}%`}

                                        {item.status === "uploaded" &&
                                            `${item.fileType?.startsWith("video/") ? "Video" : "Image"} - Uploaded`}

                                        {item.status === "failed" &&
                                            `${item.fileType?.startsWith("video/") ? "Video" : "Image"} - Failed`}
                                    </div>
